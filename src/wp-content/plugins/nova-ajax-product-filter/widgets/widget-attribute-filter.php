<?php
/**
 * Nova Ajax Product Filter by Attribute
 */
if (!class_exists('NOVAAPF_Attribute_Filter_Widget')) {
	class NOVAAPF_Attribute_Filter_Widget extends WP_Widget {
		/**
		 * Register widget with WordPress.
		 */
		function __construct() {
			parent::__construct(
				'novaapf-attribute-filter', // Base ID
				__('Nova Ajax Product Filter by Attribute', 'novaapf'), // Name
				array('description' => __('Filter woocommerce products by attribute.', 'novaapf')) // Args
			);
		}
		...
		public function widget($args, $instance) {
			...
			wp_enqueue_style('novaapf-style');
			wp_enqueue_style('font-awesome');
			wp_enqueue_script('novaapf-script');
			...
			if ($display_type === 'dropdown') {
				$output = novaapf_dropdown_terms($attr_args);
			} else {
				$output = novaapf_list_terms($attr_args);
			}
			...
			if ($found === false) {
				$widget_class = 'novaapf-widget-hidden woocommerce novaapf-ajax-term-filter';
			} else {
				$widget_class = 'woocommerce novaapf-ajax-term-filter';
			}
		...
		public function form($instance) {
			?>
			<p>
				<label for="<?php echo $this->get_field_id('title'); ?>"><?php printf(__('Title:', 'novaapf')); ?></label>
				<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo (!empty($instance['title']) ? esc_attr($instance['title']) : __( 'Filter by', 'novaapf' )); ?>">
			</p>
			<p>
			<?php
			...
			if (sizeof($attribute_taxonomies) > 0) {
				?>
				<label for="<?php echo $this->get_field_id('attr_name'); ?>"><?php printf(__('Attribute', 'novaapf')); ?></label>
				...
				printf(__('No attribute found!', 'novaapf'));
			}
			?>
			</p>
			<p>
				...
				<select class="widefat" id="<?php echo $this->get_field_id('display_type'); ?>" name="<?php echo $this->get_field_name('display_type'); ?>">
					<option value="list" <?php echo ((!empty($instance['display_type']) && $instance['display_type'] === 'list') ? 'selected="selected"' : ''); ?>><?php printf(__('List', 'novaapf')); ?></option>
					<option value="button" <?php echo ((!empty($instance['display_type']) && $instance['display_type'] === 'button') ? 'selected="selected"' : ''); ?>><?php printf(__('Button', 'novaapf')); ?></option>
					<option value="swatch" <?php echo ((!empty($instance['display_type']) && $instance['display_type'] === 'swatch') ? 'selected="selected"' : ''); ?>><?php printf(__('Swatch', 'novaapf')); ?></option>
					<option value="dropdown" <?php echo ((!empty($instance['display_type']) && $instance['display_type'] === 'dropdown') ? 'selected="selected"' : ''); ?>><?php printf(__('Dropdown', 'novaapf')); ?></option>
				</select>
			</p>
			<p>
				...
				<select class="widefat" id="<?php echo $this->get_field_id('query_type'); ?>" name="<?php echo $this->get_field_name('query_type'); ?>">
					<option value="and" <?php echo ((!empty($instance['query_type']) && $instance['query_type'] === 'and') ? 'selected="selected"' : ''); ?>><?php printf(__('AND', 'novaapf')); ?></option>
					<option value="or" <?php echo ((!empty($instance['query_type']) && $instance['query_type'] === 'or') ? 'selected="selected"' : ''); ?>><?php printf(__('OR', 'novaapf')); ?></option>
				</select>
			</p>
			<p>
				...
				<label for="<?php echo $this->get_field_id('enable_multiple'); ?>"><?php printf(__('Enable multiple filter', 'novaapf')); ?></label>
			</p>
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_count'); ?>"><?php printf(__('Show count', 'novaapf')); ?></label>
			</p>
			<p>
				...
				<label for="<?php echo $this->get_field_id('hierarchical'); ?>"><?php printf(__('Show hierarchy', 'novaapf')); ?></label>
			</p>
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_children_only'); ?>"><?php printf(__('Only show children of the current attribute', 'novaapf')); ?></label>
			</p>
			<?php
		}
...
// register widget
if (!function_exists('novaapf_register_attribute_filter_widget')) {
	function novaapf_register_attribute_filter_widget() {
		register_widget('NOVAAPF_Attribute_Filter_Widget');
	}
	add_action('widgets_init', 'novaapf_register_attribute_filter_widget');
}