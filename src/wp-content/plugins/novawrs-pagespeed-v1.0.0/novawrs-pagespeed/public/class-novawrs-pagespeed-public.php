 ...
 * @subpackage Novawrs_Pagespeed/public
 */
 ...
 */
class Novawrs_Pagespeed_Public {
	...
	public function enqueue_styles() {
		 ...
		 * defined in Novawrs_Pagespeed_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Novawrs_Pagespeed_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/novawrs-pagespeed-public.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/novawrs-pagespeed-public.js', array( 'jquery' ), $this->version, false );

	}

	public function get_default_attributes(){
		$attributes = [];
		$attributes[] = 'data-novawrspagespeed-nooptimize="true"';
	...
	protected function get_primary_script_source(){
		$content = file_get_contents(plugin_dir_path( __FILE__ ) . 'js/novawrs-pagespeed-public.js');
		...
		$extra = 'var NovawrsPageSpeedConfigs=' . json_encode($config) . ';';
...
/*        $buffer = preg_replace('/(<head\b[^>]*?>)/', '${1}' . $this->get_primary_script_source(), $buffer);*/
        ...
        while (preg_match('/<script\b[^>]*?>/is', $buffer, $matches, PREG_OFFSET_CAPTURE, $searchOffset)) {
            ...
            if (preg_match('/<\/\s*script>/is', $buffer, $endMatches, PREG_OFFSET_CAPTURE, $matches[0][1])) {
                ...
                $noOptimize = preg_match('/data-novawrspagespeed-nooptimize="true"/i', $tag);
                if ($shouldReplace && !$hasSrc && !$noOptimize) {
                    ...
                    if (apply_filters('Novawrs_Pagespeed/filter/exclude_js', false, $content)) {
                        $replacement = preg_replace('/^<script\b/i', '<script data-novawrspagespeed-nooptimize="true"', $everything);
                        $buffer = substr_replace($buffer, $replacement, $offset, $len);
                        continue;
                    }
                    $replacement = $tag . "NOVAWRSPAGESPEED[" . count($REPLACEMENTS) . "]NOVAWRSPAGESPEED" . $closingTag;
        ...
        $buffer = preg_replace_callback('/<script\b[^>]*?>/i', function ($matches) {
            ...
            if (!preg_match('/\s+data-src=/i', $result)
                && !preg_match('/data-novawrspagespeed-nooptimize="true"/i', $result)
                ...
                if ($isJavascript) {
                    if ($src) {
                        if (apply_filters('Novawrs_Pagespeed/filter/exclude_js', false, $src)) {
                            return $result;
                        }
...
//                        $result = preg_replace('/\s+async\b/i', " data-async", $result);
                    ...
                    if ($hasType) {
                        $result = preg_replace('/\s+type=([\'"])module\1/i', " type=\"javascript/blocked\" data-novawrspagespeed-module ", $result);
                        $result = preg_replace('/\s+type=module\b/i', " type=\"javascript/blocked\" data-novawrspagespeed-module ", $result);
                    ...
                    $result = preg_replace('/<script/i', "<script ${EXTRA}data-novawrspagespeed-action=\"reorder\"", $result);
                }
            }
            return preg_replace('/\s*data-novawrspagespeed-nooptimize="true"/i', '', $result);
        }, $buffer);

        $buffer = preg_replace_callback('/NOVAWRSPAGESPEED\[(\d+)\]NOVAWRSPAGESPEED/', function ($matches) use (&$REPLACEMENTS) {
