...
class Novawrs_Pagespeed {
	 ...
	 * @since    1.0.0
	 * @access   protected
	 * @var      Novawrs_Pagespeed_Loader    $loader    Maintains and registers all hooks for the plugin.
	 */
	protected $loader;
	...
	public function __construct() {
		if ( defined( 'NOVAWRS_PAGESPEED_VERSION' ) ) {
			$this->version = NOVAWRS_PAGESPEED_VERSION;
		} else {
			$this->version = '1.0.0';
		}
		$this->plugin_name = 'novawrs-pagespeed';
	 ...
	 *
	 * - Novawrs_Pagespeed_Loader. Orchestrates the hooks of the plugin.
	 * - Novawrs_Pagespeed_i18n. Defines internationalization functionality.
	 * - Novawrs_Pagespeed_Admin. Defines all hooks for the admin area.
	 * - Novawrs_Pagespeed_Public. Defines all hooks for the public side of the site.
	 *
	...
	private function load_dependencies() {
		 ...
		 * core plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-novawrs-pagespeed-loader.php';

		/**
		 * The class responsible for defining internationalization functionality
		 * of the plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-novawrs-pagespeed-i18n.php';

		/**
		 * The class responsible for looking for global method
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-novawrs-pagespeed-is-method.php';

		/**
		 * The class responsible for defining all actions that occur in the admin area.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-novawrs-pagespeed-admin.php';

		/**
		 * The class responsible for defining all actions that occur in the public-facing
		 * side of the site.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-novawrs-pagespeed-public.php';

		/**
		 * Updater
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-novawrs-pagespeed-update.php';

		$this->loader = new Novawrs_Pagespeed_Loader();
	 ...
	 *
	 * Uses the Novawrs_Pagespeed_i18n class in order to set the domain and to register the hook
	 * with WordPress.
	 *
	 * @since    1.0.0
	 * @access   private
	...
	private function set_locale() {
		...
		$plugin_i18n = new Novawrs_Pagespeed_i18n();
...
//		$plugin_admin = new Novawrs_Pagespeed_Admin( $this->get_plugin_name(), $this->get_version() );
//
...
//		$this->loader->add_action( 'admin_enqueue_scripts', $plugin_admin, 'enqueue_scripts' );
	...
	private function define_public_hooks() {
		...
		$plugin_public = new Novawrs_Pagespeed_Public( $this->get_plugin_name(), $this->get_version() );
		$this->loader->add_action('Novawrs_PageSpeed/filter/frontend_html_output', $plugin_public, 'frontend_rewrite');
	}
	 ...
	 *
	 * @since     1.0.0
	 * @return    Novawrs_Pagespeed_Loader    Orchestrates the hooks of the plugin.
	 */
    ...
    public function exclude_js_array($excluded_js) {
        ...
        return array_merge((array) $excluded_js, [
            'NovawrsPageSpeedConfigs=',
            'NovawrsPageSpeedConfigs\.',
            // 'nonovawrspagespeed=1',
        ]);
    }
	...
	protected function check_can_rewrite(){
		...
		if(isset($_GET['nonovawrspagespeed'])){
			return false;
		}
        ...
        $request = new Novawrs_Pagespeed_Is_Method();
...
//        }
		...
		return apply_filters('Novawrs_Pagespeed/filter/check_can_rewrite', $canRewrite);
	...
	public function frontend_rewrite( $output ){
		...
		$output = apply_filters('Novawrs_PageSpeed/filter/frontend_html_output', $output);
		$output = preg_replace('/<\/html>/i', "</html>\n<!-- Optimized with Novaworks PageSpeed https://novaworks.net/ -->", $output);
