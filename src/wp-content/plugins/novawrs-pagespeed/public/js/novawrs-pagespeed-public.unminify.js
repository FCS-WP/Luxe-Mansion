class NovawrsPageSpeedClass {
    ...
    async load_resources() {
        ...
        this.add_html_class("novasf_ps-start");
        ...
        window.dispatchEvent(new Event("NovawrsPageSpeed:Loaded"));
        this.add_html_class("novasf_ps-start_js");
        await this.load_scripts(this.enqueued_scripts.lazy);
        this.add_html_class("novasf_ps-js_loaded");
    }
    ...
    register_scripts() {
        document.querySelectorAll("script[data-novawrspagespeed-action=reorder]").forEach((e => {
            if(e.hasAttribute("src") || e.hasAttribute("data-src")){
                if(e.hasAttribute("async") && false !== e.async){
                    this.enqueued_scripts.async.push(e)
                }
                else{
                    if(e.hasAttribute("defer") && false !== e.defer || "module" === e.getAttribute("data-novawrspagespeed-module")){
        ...
        document.querySelectorAll("script[data-novawrspagespeed-action=lazyload_ext]").forEach((e => {
    ...
    async execute_script(e) {
        return await this.repaint_frame(), new Promise((t => {
            ...
            [...e.attributes].forEach((e => {
                ...
                if("type" !== t && "data-novawrspagespeed-action" !== t){
                    r = e.nodeValue;
                    if("data-novawrspagespeed-module" === t){
                        t = "type";
                        r = "module";
                    }
                    if("data-src" === t){
    ...
    hold_event_listeners() {
        ...
        function t(t, n) {
            !function (t) {
                function n(n) {
                    return e[t].eventsToRewrite.indexOf(n) >= 0 ? "novafps-" + n : n
                }
                e[t] || (e[t] = {
                    originalFunctions: {
            ...
            set(n) {
                if (n && n.fn && n.fn.on && !e.allJQueries.includes(n)) {
                    n.fn.ready = n.fn.init.prototype.ready = function (t) {
                        e.domReadyFired ? t.bind(document)(n) : document.addEventListener("novafps-DOMContentLoaded", (() => t.bind(document)(n)))
                    };
                    const t = n.fn.on;
                    n.fn.on = n.fn.init.prototype.on = function () {
                        if (this[0] === window) {
                            function e(e) {
                                return e.split(" ").map((e => "load" === e || 0 === e.indexOf("load.") ? "novasf_ps-jquery_loaded" : e)).join(" ")
                            }
    ...
    async execute_domcontentloaded() {
        ...
        document.dispatchEvent(new Event("novafps-DOMContentLoaded"));
        await this.repaint_frame();
        window.dispatchEvent(new Event("novafps-DOMContentLoaded"));
        await this.repaint_frame();
        document.dispatchEvent(new Event("novafps-readystatechange"));
        await this.repaint_frame();
        document.novafpsonreadystatechange && document.novafpsonreadystatechange()
    }

    async execute_window_load() {
        await this.repaint_frame();
        window.dispatchEvent(new Event("novafps-load"));
        await this.repaint_frame();
        window.novafpsonload && window.novafpsonload();
        await this.repaint_frame();
        this.allJQueries.forEach((e => e(window).trigger("novasf_ps-jquery_loaded")));
        window.dispatchEvent(new Event("novafps-pageshow"));
        await this.repaint_frame();
        window.novafpsonpageshow && window.novafpsonpageshow()
    }
    ...
    static runoptimize() {
        const ps_instance = new NovawrsPageSpeedClass(["keydown", "mousemove", "touchmove", "touchstart", "touchend", "wheel"]);
        ps_instance.load_fonts(ps_instance.enqueued_fonts);
        ps_instance.user_events_add(ps_instance)
        if(NovawrsPageSpeedConfigs.detected){
            if(ps_instance.is_pagespeed()){
                ps_instance.add_html_class('isPageSpeed');
                setTimeout(function (e){
                    if(!document.documentElement.classList.contains('novasf_ps-start')){
                        e.triggerListener()
                    }
                }, NovawrsPageSpeedConfigs.delay, ps_instance)
            }
            else{
...
NovawrsPageSpeedClass.runoptimize();

window.addEventListener('NovawrsPageSpeed:Loaded', () => {
